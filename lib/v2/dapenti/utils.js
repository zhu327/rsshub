...
module.exports = {
    parseFeed: async (ctx) => {
        const url = `https://www.dapenti.com/blog/blog.asp?name=xilei`;
        ...
        // 只取最近的三个，取全文rss
        const list = $('li', 'ul').filter(function () {
            const anchorText = $(this).find('a').text();
            return anchorText.includes('喷嚏图卦');
        }).slice(0, 3).get();
