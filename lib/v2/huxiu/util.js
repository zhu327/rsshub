...
const { renderDescription } = require('./templates/description');
...
const cleanUpHTML = (data) => {
    ...
    $('p img').each((_, el) => {
        const img = $(el);
        const src = img.prop('src') ?? img.prop('_src');
        if (src === undefined) {
            return;
        }
        img.parent().replaceWith(
            renderDescription({
                image: {
                    ...
                    width: img.prop('data-w'),
                    height: img.prop('data-h'),
                },
            })
        );
    });
    $('p, span').each((_, e) => {
        ...
        if (e.contents().length === 1 && /^\s*$/.test(e.text())) {
            ...
            e.removeClass();
            e.removeAttr('data-check-id').removeAttr('label').removeAttr('class');
        }
    });
    ...
    if (nuxtDataScript) {
        try {
            ...
            for (const item of nuxtData) {
                ...
                if (!Object.hasOwn(item, 'fetchData') || typeof item.fetchData !== 'number') {
                    continue;
                }
                clubData = resolveNuxtData(nuxtData, item.fetchData);
                break;
            }
        } catch {
...
const buildCategories = (data) =>
    [
        data.video_article_tag,
        data.brief_column?.name,
        data.club_info?.name,
        ...(data.tags_info?.map((c) => c.name) ?? []),
        ...(data.relation_info?.channel?.map((c) => c.name) ?? []),
    ].filter(Boolean);

/**
 * 从文章 URL 中提取 aid（文章 ID）。
 *
 * @param {string} link - 文章链接，如 https://www.huxiu.com/article/4846357.html
 * @returns {string|null} - 文章 ID 或 null
 */
const extractAid = (link) => {
    const match = link.match(/\/article\/(\d+)/);
    return match ? match[1] : null;
};

/**
 * 通过 API 获取文章详情（绕过 WAF 验证）。
 *
 * @param {Object} item - The item to fetch data for.
 * @returns {Promise<Object>} The fetched item data object.
 */
const fetchItem = async (item) => {
    const aid = extractAid(item.link);
    if (!aid) {
        return item;
    }
    ...
    return {
        ...
        author: data.user_info?.username ?? data.author ?? item.author,
 ...
 *
 * @returns {Object} - { nonce, timestamp, signature }
 */
const generateSignature = () => {
    ...
    const sorted = [appSecret, timestamp, nonce].sort();
    return {
        nonce,
        timestamp,
        signature: CryptoJS.SHA1(sorted[0] + sorted[1] + sorted[2]).toString(),
    };
};
 ...
 */
const resolveNuxtData = (arr, index, visited = new Set()) => {
    ...
    if (Array.isArray(item) && (item[0] === 'ShallowReactive' || item[0] === 'Reactive') && typeof item[1] === 'number') {
...
const parseInitialState = (data) => {
    ...
    if (nuxtDataScript) {
        try {
            ...
            for (const item of nuxtData) {
                ...
                if (!articleDetailKey || typeof item[articleDetailKey] !== 'number') {
                    continue;
                }
                const articleData = resolveNuxtData(nuxtData, item[articleDetailKey]);
    ...
    // window.__INITIAL_STATE__ 格式（简报页）
    const initialStateMatch = data.match(/window\.__INITIAL_STATE__\s*=\s*(\{[\s\S]*?\});?\s*\(function\(\)/);
    if (initialStateMatch?.[1]) {
        try {
            const initialState = JSON.parse(initialStateMatch[1]);
            const briefDetail = initialState.briefStoreModule?.brief_detail;
            const brief = briefDetail?.brief;
            if (!brief) {
                return undefined;
            }

            const publisher = brief.publisher_list?.[0];
            const briefColumn = briefDetail.brief_column;
            const clubInfo = briefDetail.club_info;
            ...
            return {
                articleDetail: {
                    articleDetail: {
                        ...
                        user_info: publisher ? { username: publisher.username } : undefined,
...
const audioPathKeyForQuality = (q) => (q === '' ? 'audio_path' : `audio_${q}_path`);
...
const processAudioInfo = (info) => {
    const quality = info ? audioQualities.find((q) => Object.hasOwn(info, audioPathKeyForQuality(q))) : undefined;
    ...
    return await Promise.all(
        processedItems.map((item) =>
            tryGet(item.guid, async () => {
                ...
                try {
                    isExternalLink = !new RegExp(domain, 'i').test(new URL(item.link).hostname);
                } catch {
                    return item;
                }
