...
const fetchItem = async (item) => {
    ...
    const data = state?.briefStoreModule?.brief_detail?.brief ?? state?.articleDetail?.articleDetail ?? state?.articleDetail ?? undefined;
...
/**
 * Resolves Nuxt 3 payload references recursively.
 *
 * @param {Array} payload - The Nuxt 3 payload array.
 * @param {number|any} index - The index to resolve or the value itself.
 * @param {Set} visited - Set of visited indices to prevent infinite recursion.
 * @returns {any} - The resolved value.
 */
const resolveNuxtPayload = (payload, index, visited = new Set()) => {
    ...
    // Handle special Nuxt markers like ["ShallowReactive", 1], ["Set"], ["Reactive", ...]
    if (Array.isArray(value)) {
        if (value.length >= 1 && typeof value[0] === 'string') {
            const marker = value[0];
            if (marker === 'ShallowReactive' || marker === 'Reactive') {
                return resolveNuxtPayload(payload, value[1], visited);
            }
            if (marker === 'Set') {
                return new Set();
            }
        ...
        return value.map((v) => resolveNuxtPayload(payload, v, new Set(visited)));
    }
    ...
    if (typeof value === 'object') {
        ...
        for (const [key, val] of Object.entries(value)) {
            resolved[key] = resolveNuxtPayload(payload, val, new Set(visited));
        }
        return resolved;
    }
...
/**
 * Parses article content directly from HTML when other methods fail.
 *
 * @param {string} html - The HTML content of the article page.
 * @returns {Object|undefined} - The parsed article data, or undefined if not found.
 */
const parseArticleFromHTML = (html) => {
    const $ = cheerio.load(html);

    // Try to find article content using common selectors
    const articleContent = $('.article-content-wrap, .article__content, .article-content, .content-wrap').html();
    const title = $('h1.article-title, h1.title, .article__title h1').text().trim() || $('title').text().split('-')[0].trim();
    const author = $('.author-name, .article-author, .author__name').text().trim() || $('meta[name="author"]').attr('content');
    const pubDate = $('meta[property="article:published_time"]').attr('content') || $('time').attr('datetime');
    const summary = $('meta[name="description"]').attr('content');
    const picPath = $('meta[property="og:image"]').attr('content');
    ...
    if (articleContent) {
        return {
            ...
            publish_time: pubDate ? Math.floor(new Date(pubDate).getTime() / 1000) : undefined,
            preface: summary,
            pic_path: picPath,
        };
    }
 ...
 * Supports both legacy window.__INITIAL_STATE__ format and Nuxt 3 __NUXT_DATA__ format.
 *
...
const parseInitialState = (data) => {
    // Try legacy format first: window.__INITIAL_STATE__
    const legacyMatches = data.match(/window\.__INITIAL_STATE__=({.*?});\(function\(\)/);
    if (legacyMatches) {
        try {
            return JSON.parse(legacyMatches[1]);
        } catch {
            // JSON parse failed, continue to next method
        }
    }

    // Try alternative INITIAL_STATE format (without the function suffix)
    const altMatches = data.match(/window\.__INITIAL_STATE__\s*=\s*({[\s\S]*?});?\s*<\/script>/);
    if (altMatches) {
        try {
    ...
    // Try Nuxt 3 format: __NUXT_DATA__ script tag
    const $ = cheerio.load(data);
    const nuxtDataScript = $('#__NUXT_DATA__').html();

    if (nuxtDataScript) {
        try {
            const payload = JSON.parse(nuxtDataScript);
            if (Array.isArray(payload) && payload.length > 1) {
                // The root object is usually at index 1 after the first marker
                const rootData = resolveNuxtPayload(payload, 1);
                if (rootData && rootData.data) {
                    // Extract articleDetail from the resolved data
                    const dataSection = rootData.data;
                    // Find the articleDetail key (e.g., "articleDetail-4810965")
                    for (const key of Object.keys(dataSection)) {
                        if (key.startsWith('articleDetail-')) {
                            const articleData = dataSection[key];
                            // Return structure compatible with original code expectation
                            return {
                                articleDetail: {
                        ...
                        if (key.startsWith('briefDetail-')) {
                            const briefData = dataSection[key];
                            return {
                                briefStoreModule: {
                                    brief_detail: {
                                        brief: briefData,
    ...
    // Fallback: parse article directly from HTML
    const articleData = parseArticleFromHTML(data);
    if (articleData) {
        return {
            articleDetail: {
                articleDetail: articleData,
