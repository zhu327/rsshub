...
module.exports = async (ctx) => {
    ...
    const seenMessageTitles = username === 'niuyueshibao_rss' ? new Set() : null;
        ...
        item:
            ...
            list
                .map((index, item) => {
                    ...
                    let link = item.find('.tgme_widget_message_date').attr('href');
                    if (username === "niuyueshibao_rss" || username === "wsj_rss" || username === "ttww_rss") {
                        link = item.find('.tgme_widget_message_link_preview').attr('href')

                        if (!link || !link.startsWith('https://telegra.ph/')) {
                            const linkElement = item.find('a').filter(function() {
                                return $(this).attr('href').startsWith('https://telegra.ph/');
                            });
                    ...
                    if (username === "wsj_rss") {
                        const titleElement = item.find('.link_preview_title');

                        if (titleElement.length > 0 && titleElement.text().trim() !== "") {
                            messageTitle = titleElement.text().trim();
                        }
                    };

                    if (username === "niuyueshibao_rss" || username === "ttww_rss") {
                        const suffix = "Telegraph | 原文链接 | 频道大全";
                        if (messageTitle.endsWith(suffix)) {
                            messageTitle = messageTitle.slice(0, -suffix.length).trim();
                        };
                    }

                    if (username === 'niuyueshibao_rss' && seenMessageTitles) {
                        if (seenMessageTitles.has(messageTitle)) {
                            return null; // drop duplicate messages sharing the same title
                        }
                        seenMessageTitles.add(messageTitle);
                    }
